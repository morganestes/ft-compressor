	...
	class FTCompressor
	{
		public static string CompressString(string s)
		{
			...
			for (int i = 0; i < s.Length; i += 2) {
				...
				int intStr1 = Char.ConvertToUtf32(s, i) * 256;
				int intStr2 = Char.ConvertToUtf32(s, i + 1);
		...
		public static string DecompressString(string s)
		{
			...
			for (int i = 1, l = s.Length; i < l; i++) {
				int n = Char.ConvertToUtf32(s, i);
				int m = (int)Math.Floor((decimal)n / 256);

				strOut.Append(Char.ConvertFromUtf32(m) + Char.ConvertFromUtf32(n % 256));
			}
